...
class EvaluateFile(Command):
    # Parser configuration
    summary = "Administration functions for LODStats"
    usage = "paster-2.6 --plugin=Rdfstats evaluate_file"
    group_name = "rdfstats"
    ...
    def command(self):
        ...
        rdfdoc = model.RDFDoc()
        rdfdoc.name = '' #ckan name
        rdfdoc.uri = '' #ckan uri
        rdfdoc.format = 'nt' # nt
        
        rdfdoc = Session.query(model.RDFDoc).filter(model.RDFDoc.name=="world-bank-linked-data").first()
        print rdfdoc
